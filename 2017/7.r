...
; --- Part 2 ---
corrected-tree: make tree [
  corrected-weight: 0
  get-corrected-weight: does [find-error get-root corrected-weight]
  find-error: func [name [string!] /local entry result weights values error delta] [
    ...
    if all [
      ...
      corrected-weight: (third tree/get-entry pick entry/4 index? error) - delta
    ]
    result
  ]
]
print [{Part 2:} r2: corrected-tree/get-corrected-weight]
