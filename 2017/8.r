...
; --- Part 1 ---
state: context init: unique/skip collect [
  forskip data 7 [
    keep reduce [to-set-word data/1 0]
  ]
] 2
rule: [
  set reg word!
  set op ['inc | 'dec] (op: either op == 'inc ['add] ['subtract])
  set value [integer!]
  copy cond 4 any-type! (do bind repend/only cond [to-set-word reg op reg value] state)
]
parse data [any rule]
print [{Part 1:} r1: first maximum-of next second state]

; --- Part 2 ---
state: context append copy init [max: 0]
append last max-rule: copy rule [state/max: max state/max state/:reg]
parse data [any max-rule]
print [{Part 2:} r2: state/max]
